...
export function SingleCard({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      className={`w-full h-full bg-[rgba(255,_255,_255,0.1)] flex justify-center items-center relative p-[2px] before:[background-image:radial-gradient(800px_circle_at_var(--mouse-x)_var(--mouse-y),_rgba(255,_255,_255,_0.30),_transparent_40%)] before:absolute before:top-0 before:left-0 before:h-full before:w-full before:transition-all before:delay-75 before:opacity-0 ${
        isMouseOnCard ? "before:opacity-100" : ""
      }`}
      onMouseMove={setCssVariantsForBorder}
    >
      <article
        className={`max-w-5xl w-full h-full bg-[#232227] flex flex-col items-start justify-center relative overflow-hidden gap-4 z-[1] ${
          isMouseOnCard &&
        ...
        } before:backdrop-blur-md before:absolute before:top-0 before:left-0 before:h-full before:w-full before:z-[0] before:transition-all before:delay-75 before:opacity-0 ${
          isMouseOnCard ? "before:opacity-100" : ""
        }`}
