...
export function SingleCard({ children }: { children: ReactNode }) {
  ...
  return (
    ...
    >
      <article
        className={`max-w-5xl w-full h-full bg-[#1F2125] flex flex-col items-start justify-center relative overflow-hidden px-32 py-12 rounded-xl gap-4 z-[1] before:[background-image:radial-gradient(1200px_circle_at_var(--mouse-x)_var(--mouse-y),_rgba(255,_254,_241,_0.06),_transparent_40%)] before:rounded-xl before:backdrop-blur-md before:absolute before:top-0 before:left-0 before:h-full before:w-full before:z-[0] before:transition-all before:delay-75 before:opacity-0 ${
          isMouseOnCard ? "before:opacity-100" : ""
        }`}
